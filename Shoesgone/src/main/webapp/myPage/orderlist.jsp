...
<div module="Myshop_OrderOldHistoryList">
        ...
        <tbody class="{$list_display|display}">
            <%
						...
						for(int i = 0; i < list.size(); i++){
							if(loginMember.getUserNo() == list.get(i).getBuyerNo()){
					%>
  ...
  <div style="text-align:center;">
	<% if(currentPage <= 1){ %>
		[맨처음] &nbsp;
	<% }else{ %>
		<a href="/Shoesgone/olist?page=1">[맨처음]</a> &nbsp;
	<% } %>
	<!-- 이전 페이지 그룹으로 이동 -->
	<% if((currentPage - 10) < startPage && 
			(currentPage - 10) > 1){ %>
		<a href="/Shoesgone/olist?page=<%= startPage - 10 %>">[이전그룹]</a> &nbsp;
	<% }else{ %>
		[이전그룹] &nbsp;
	<% } %>
	...
	%>
		...
		<% }else{ %>
			<a href="/Shoesgone/olist?page=<%= p %>"><%= p %></a>
	<% }} %>
	
	<!-- 다음 페이지 그룹으로 이동 -->
	<% if((currentPage + 10) > endPage && 
			(currentPage + 10) < maxPage){ %>
		<a href="/Shoesgone/olist?page=<%= endPage + 10 %>">[다음그룹]</a> &nbsp;
	<% }else{ %>
		[다음그룹] &nbsp;
	<% } %>
	...
	<% if(currentPage >= maxPage){ %>
		[맨끝] &nbsp;
	<% }else{ %>
		<a href="/Shoesgone/olist?page=<%= maxPage %>">[맨끝]</a> &nbsp;
	<% } %>
</div>
<center><a href="/Shoesgone/contact.html" class="genric-btn primary">목록보기</a></center>
<hr>
    <br><br><br>
</div>
</section>
