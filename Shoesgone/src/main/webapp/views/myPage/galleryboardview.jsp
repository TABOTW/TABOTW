<%@ page language="java" contentType="text/html; charset=UTF-8"
  ...
  <%
  ArrayList<Myboard> glist = (ArrayList<Myboard>)request.getAttribute("glist");

  int flistCount = ((Integer)request.getAttribute("glistCount")).intValue();
				...
				<table class="sub_news" border="0" cellpadding="7px"
					...
					%>
								<tr>
									...
									<td class="title" style = "text-align:center"><a href="/Shoesgone/menubarsearch?menu=gallerydetail<%= glist.get(i).getTextNo() %>"><%= glist.get(i).getTitle() %></td>
...
<div style="text-align:center;">
	<% if(currentPage <= 1){ %>
		[맨처음] &nbsp;
	<% }else{ %>
		<a href="/Shoesgone/menubarsearch?menu=glistmy<%= loginMember.getUserId() %>&page=1">[맨처음]</a> &nbsp;
	<% } %>
	<!-- 이전 페이지 그룹으로 이동 -->
	<% if((currentPage - 10) < startPage && 
			(currentPage - 10) > 1){ %>
		<a href="/Shoesgone/menubarsearch?menu=glistmy<%= loginMember.getUserId() %>&page=<%= startPage - 10 %>">[이전그룹]</a> &nbsp;
	<% }else{ %>
		[이전그룹] &nbsp;
	<% } %>
	...
	%>
		...
		<% }else{ %>
			<a href="/Shoesgone/menubarsearch?menu=glistmy<%= loginMember.getUserId() %>&page=<%= p %>"><%= p %></a>
	<% }} %>
	
	<!-- 다음 페이지 그룹으로 이동 -->
	<% if((currentPage + 10) > endPage && 
			(currentPage + 10) < maxPage){ %>
		<a href="/Shoesgone/menubarsearch?menu=glistmy<%= loginMember.getUserId() %>&page=<%= endPage + 10 %>">[다음그룹]</a> &nbsp;
	<% }else{ %>
		[다음그룹] &nbsp;
	<% } %>
	...
	<% if(currentPage >= maxPage){ %>
		[맨끝] &nbsp;
	<% }else{ %>
		<a href="/Shoesgone/menubarsearch?menu=glistmy<%= loginMember.getUserId() %>&page=<%= maxPage %>">[맨끝]</a> &nbsp;
	<% } %>
</div>
