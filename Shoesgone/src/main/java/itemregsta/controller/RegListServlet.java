...
/**
 * Servlet implementation class RegListServlet
 */
@WebServlet("/reglist")
public class RegListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ItemRegStaService regservice = new ItemRegStaService();
		int regListCount = regservice.getRegListCount();
		...
		ArrayList<ItemRegSta> list = regservice.selectRegList(startRow, endRow);
		
		// 출력할 페이지 관련 숫자 처리
		int maxPage = (int)((double)regListCount / pageLimit + 0.9);
		...
		if(list.size() > 0) {
			view = request.getRequestDispatcher("나중에 제품 보기 jsp 파일의 주소를 상대경로로 작성해주세요.");
			request.setAttribute("list", list);
			request.setAttribute("currentPage", currentPage);
			request.setAttribute("maxPage", maxPage);
			request.setAttribute("startPage", startPage);
			request.setAttribute("endPage", endPage);
			request.setAttribute("regListCount", regListCount);
		} else {
