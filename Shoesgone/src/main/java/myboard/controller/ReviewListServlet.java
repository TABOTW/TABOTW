...
public class ReviewListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int rlistCount = mservice.getReviewListCount(userno);
		...
		ArrayList<Myboard> rlist = mservice.reviewList(userno, startRow, endRow);
		
		//뷰 페이지로 같이 내보낼 페이지 관련 숫자 계산 처리
		//총 페이지 수 : 총 목록이 21개인 경우
		//한 페이지에 출력할 목록이 10개이면, 페이지는 3임
		int maxPage = (int)((double)rlistCount / limit + 0.9);
