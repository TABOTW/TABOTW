...
public class ReviewListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String orderBy = " REVIEW_NO DESC "; // 기본은 글 최신 순
		if(sort.equals("likeup")) {
			orderBy = " REVIEW_LIKE DESC ";
		} else if (sort.equals("likedown")) {
			orderBy = " REVIEW_LIKE ASC ";
		} else if (sort.equals("newest")) {
			orderBy = " REVIEW_NO DESC ";
		} else if (sort.equals("oldest")) {
			orderBy = " REVIEW_NO ASC";
		}
		...
		ArrayList<Review> list = bservice.selectList(startRow, endRow, orderBy);
		...
		if(list.size() > 0) {
			view = request.getRequestDispatcher(
					"views/community/review/reviewListView.jsp");
