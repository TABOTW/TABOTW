...
public class BestListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String orderBy = " BEST_NO DESC "; // 기본은 글 최신 순
		if(sort.equals("likeup")) {
			orderBy = " BEST_LIKE DESC ";
		} else if (sort.equals("likedown")) {
			orderBy = " BEST_LIKE ASC ";
		} else if (sort.equals("newest")) {
			orderBy = " BEST_NO DESC ";
		...
		ArrayList<Best> list = bservice.selectList(startRow, endRow, orderBy);
		...
		if(list.size() > 0) {
			view = request.getRequestDispatcher(
					"views/community/best/bestListView.jsp");
